...
///
/// Manager class that extends RHDatagram to define addressed, reliable datagrams with acknowledgement and
/// retransmission. Based on RHDatagram, adds flags and sequence numbers. RHReliableDatagram is reliable in the sense
/// that messages are acknowledged by the recipient, and unacknowledged messages are retransmitted until acknowledged or
/// the retries are exhausted. When addressed messages are sent (by sendtoWait()), it will wait for an ack, and
/// retransmit after timeout until an ack is received or retries are exhausted. When addressed messages are collected by
/// the application (by recvfromAck()), an acknowledgement is automatically sent to the sender.
///
...
class RHReliableDatagram : public RHDatagram
{
  public:
    ...
    /// \param[in,out] len Pointer to the number of octets available in buf. The number be reset to the actual number of
    /// octets copied. \param[in] from If present and not NULL, the referenced uint8_t will be set to the SRC address
    ...
    /// \param[in] buf Location to copy the received message
    /// \param[in,out] len Pointer to the number of octets available in buf. The number be reset to the actual number of
    /// octets copied. \param[in] timeout Maximum time to wait in milliseconds \param[in] from If present and not NULL,
    /// the referenced uint8_t will be set to the SRC address \param[in] to If present and not NULL, the referenced
    /// uint8_t will be set to the DEST address \param[in] id If present and not NULL, the referenced uint8_t will be
    /// set to the ID \param[in] flags If present and not NULL, the referenced uint8_t will be set to the FLAGS (not
    /// just those addressed to this node). \return true if a valid message was copied to buf
