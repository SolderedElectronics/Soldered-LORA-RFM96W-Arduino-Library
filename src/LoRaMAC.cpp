                ...
                sLoRa_OTAA *OTAA_Data, sLoRa_Message *Message_Rx, sSettings *LoRa_Settings, msg_t *upMsg_Type)
{
    ...
    if (*RFM_Command == NEW_RFM_COMMAND)
    {
        ...
        // Lora send data & ack
        if (*upMsg_Type == MSG_UP)
            LORA_Send_Data(Data_Tx, Session_Data, LoRa_Settings);
        else if (*upMsg_Type == MSG_ACK)
            LORA_Send_ACK(Data_Tx, Session_Data, LoRa_Settings);
...
/* Added the band AU_915 for use in class C */
            ...
            LORA_Receive_Data(Data_Rx, Session_Data, OTAA_Data, Message_Rx,
                              LoRa_Settings); // BUG DETECT SENDED PACKET ALWAYS (IT DOES UPDATE)
        }
                       ...
                       sSettings *LoRa_Settings)
{
    ...
    if (Message_Status == CRC_OK)
    {
        ...
        // Data message
        if (Message->MAC_Header == 0x40 || Message->MAC_Header == 0x60 || Message->MAC_Header == 0x80 ||
            Message->MAC_Header == 0xA0)
        {
...
bool LORA_join_Accept(sBuffer *Data_Rx, sLoRa_Session *Session_Data, sLoRa_OTAA *OTAA_Data, sLoRa_Message *Message,
                      sSettings *LoRa_Settings)
{
