...
{
    Serial.print(F(" rps=0x"));
    printHex2(rps);
    Serial.print(F(" ("));
    Serial.print(getSfName(rps));
    printSpace();
    Serial.print(getBwName(rps));
    printSpace();
    Serial.print(getCrName(rps));
    printSpace();
    Serial.print(getCrcName(rps));
    Serial.print(F(" IH="));
    Serial.print(unsigned(getIh(rps)));
    Serial.print(')');
}
...
void printTxend(cEventQueue::eventnode_t &e)
{
    Serial.print(F(", txend="));
    Serial.print(e.txend);
    Serial.print(F(", avail="));
    Serial.print(e.globalDutyAvail);
}

void printRxtime(cEventQueue::eventnode_t &e)
{
    Serial.print(F(", rxtime="));
    Serial.print(e.rxtime);
}
...
void printDatarate(u1_t datarate)
{
    ...
    Serial.print(unsigned(datarate));
}
...
void eventPrint(cEventQueue::eventnode_t &e)
{
    ...
    if (ev == ev_t(-1) || ev == ev_t(-2))
    {
        ...
        if (ev == ev_t(-2))
        {
            Serial.print(F(", datum=0x"));
            Serial.print(e.datum, HEX);
        }
    ...
    else
    {
        ...
        */
        ...
        case EV_RXSTART:
            ...
            Serial.print(unsigned(e.rxsyms));
            break;
...
{
    return p[0] == '\0' ? s : (p[0] == '/' || p[0] == '\\') ? filebasename2(p + 1, p + 1) : filebasename2(s, p + 1);
}
...
void loop()
{
    ...
    if ((LMIC.opmode & OP_TXRXPEND) == 0 && !os_queryTimeCriticalJobs(ms2osticks(1000)))
    {
        eventPrintAll();
    }
}
...
static constexpr bool kMustCalibrateLSE =
    NEED_STM32_ClockCalibration; // _mcci_arduino_version indicates that LSE clock is used.
...
bool rtcbegin()
{
    ...
    else
    {
        ...
        */
        Serial.println(" HSE can not be used for RTC clock!");
        return false;
    }
    ...
    if (HAL_RTCEx_BKUPRead(&hRtc, RTC_BKP_DR0) != 0x32F2)
    {
        ...
        {
            Serial.print("HAL_RTC_SetTime() failed");
            return false;
        }
        ...
        {
            Serial.print("HAL_RTC_SetDate() failed");
            return false;
        }
...
uint32_t Stm32_CalibrateSystemClock(void)
{
    ...
    {
        // Use original value
        Serial.println("?CalibrateSystemClock: can't calibrate");
    }
